...
logger = logging.getLogger("campaigniq.tasks")
...
@celery_app.task(bind=True, name="campaigniq.tasks.validate_url_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.discover_pages_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.crawl_pages_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.extract_and_brand_task", max_retries=0)
def extract_and_brand_task(self, prev: dict) -> dict:
    ...
    try:
        ...
        # Infer voice only after enough extracted site copy exists. Failure to
        # infer style must not fail the crawl or discard visual branding.
        style_source = "\n\n".join(page.clean_text or "" for page in pages)
        if len(style_source.strip()) >= 500:
            try:
                from app.services.writing_style import derive_writing_style
                style = asyncio.run(derive_writing_style(style_source))
                if style:
                    style_brand = db.execute(
                        select(BrandSettings).where(BrandSettings.bot_id == uuid.UUID(bot_id))
                    ).scalar_one_or_none()
                    if style_brand:
                        style_brand.writing_style = style
                        db.commit()
                        logger.info("[extract_and_brand] writing style saved: fields=%d", len(style))
            except Exception as style_exc:
                logger.warning("[extract_and_brand] writing style inference failed: %s", style_exc)
...
@celery_app.task(bind=True, name="campaigniq.tasks.compile_knowledge_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.chunk_and_embed_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.finalize_crawl_task", max_retries=0)
...
@celery_app.task(bind=True, name="campaigniq.tasks.send_recipient", max_retries=0)
def send_recipient_task(self, recipient_id: str) -> dict:
    """Send one initial email; failures are isolated to this recipient."""
    ...
    db = _get_sync_session()
    recipient = db.get(CampaignRecipient, uuid.UUID(recipient_id))
    try:
        if not recipient:
            return {"recipient_id": recipient_id, "status": "missing"}
        campaign = db.get(Campaign, recipient.campaign_id)
        contact = db.get(Contact, recipient.contact_id)
        step = db.get(CampaignStep, recipient.step_id)
        if not campaign or not contact or not step or step.step_order != 1 or recipient.status != "GENERATED":
            return {"recipient_id": recipient_id, "status": "skipped"}
        recipient.status = "SENDING"
        db.commit()
        try:
            get_email_provider().send(contact.email, recipient.subject or "", recipient.body or "")
            recipient.status = "SENT"
            recipient.sent_at = datetime.now(timezone.utc)
            recipient.error = None
            db.commit()
            _schedule_next_step(db, campaign, contact, step)
            _derive_campaign_send_status(db, campaign.id)
            return {"recipient_id": recipient_id, "status": "SENT"}
        except Exception as exc:
            recipient.status = "FAILED"
            recipient.error = str(exc)[:2000]
            db.commit()
            _derive_campaign_send_status(db, campaign.id)
            return {"recipient_id": recipient_id, "status": "FAILED"}
    finally:
        db.close()


def _schedule_next_step(db: Session, campaign, contact, step) -> None:
    next_step = db.execute(select(CampaignStep).where(CampaignStep.campaign_id == campaign.id, CampaignStep.step_order == step.step_order + 1)).scalar_one_or_none()
    if not next_step:
        return
    next_recipient = db.execute(select(CampaignRecipient).where(CampaignRecipient.campaign_id == campaign.id, CampaignRecipient.contact_id == contact.id, CampaignRecipient.step_id == next_step.id)).scalar_one_or_none()
    if next_recipient:
        send_followup_task.apply_async(args=[str(next_recipient.id)], countdown=next_step.delay_days * 86400)


def _derive_campaign_send_status(db: Session, campaign_id: uuid.UUID) -> None:
    from app.models.campaign import Campaign
    from app.models.campaign_recipient import CampaignRecipient
    from app.models.campaign_step import CampaignStep
    ...
    if not campaign:
        return
    first_step = db.execute(select(CampaignStep).where(CampaignStep.campaign_id == campaign_id, CampaignStep.step_order == 1)).scalar_one()
    statuses = [row.status for row in db.execute(select(CampaignRecipient).where(CampaignRecipient.campaign_id == campaign_id, CampaignRecipient.step_id == first_step.id)).scalars().all()]
    if not statuses or any(status in {"GENERATED", "SENDING", "PENDING"} for status in statuses):
        return
    sent = sum(status == "SENT" for status in statuses)
    failed = sum(status == "FAILED" for status in statuses)
    campaign.status = "SENT" if sent and not failed else ("PARTIAL_FAILED" if sent else "FAILED")
    db.commit()


@celery_app.task(bind=True, name="campaigniq.tasks.send_followup", max_retries=0)
def send_followup_task(self, recipient_id: str) -> dict:
    """Generate and send a follow-up only after its preceding step succeeded."""
    import asyncio
    ...
    from app.models.campaign_step import CampaignStep
    from app.models.contact import Contact
    from app.services.campaign_service import generate_recipient_email
    from app.services.email_provider import get_email_provider
    ...
    try:
        ...
        campaign = db.get(Campaign, recipient.campaign_id)
        step = db.get(CampaignStep, recipient.step_id)
        previous = db.execute(select(CampaignStep).where(CampaignStep.campaign_id == campaign.id, CampaignStep.step_order == step.step_order - 1)).scalar_one_or_none() if campaign and step else None
        prior_recipient = db.execute(select(CampaignRecipient).where(CampaignRecipient.campaign_id == campaign.id, CampaignRecipient.contact_id == recipient.contact_id, CampaignRecipient.step_id == previous.id)).scalar_one_or_none() if previous else None
        contact = db.get(Contact, recipient.contact_id)
        if not campaign or not step or not contact or not prior_recipient or prior_recipient.status != "SENT" or recipient.status != "PENDING":
            return {"recipient_id": recipient_id, "status": "skipped"}
        recipient.status = "SENDING"
        db.commit()
        ...
        try:
            async def generate():
                async with async_sessionmaker(async_engine, expire_on_commit=False)() as async_db:
                    return await generate_recipient_email(recipient, campaign, step, async_db)
            subject, body = asyncio.run(generate())
        finally:
            asyncio.run(async_engine.dispose())
        get_email_provider().send(contact.email, subject, body)
        recipient.subject, recipient.body, recipient.status = subject, body, "SENT"
        recipient.sent_at = datetime.now(timezone.utc)
        db.commit()
        return {"recipient_id": recipient_id, "status": "SENT"}
    except Exception as exc:
        if recipient:
            recipient.status, recipient.error = "FAILED", str(exc)[:2000]
            db.commit()
        return {"recipient_id": recipient_id, "status": "FAILED"}
    finally:
        db.close()
...
def enqueue_campaign_send(campaign_id: uuid.UUID) -> int:
    from app.models.campaign_recipient import CampaignRecipient
    from app.models.campaign_step import CampaignStep
    db = _get_sync_session()
    try:
        first_step = db.execute(select(CampaignStep).where(CampaignStep.campaign_id == campaign_id, CampaignStep.step_order == 1)).scalar_one()
        recipients = db.execute(select(CampaignRecipient).where(CampaignRecipient.campaign_id == campaign_id, CampaignRecipient.step_id == first_step.id, CampaignRecipient.status == "GENERATED")).scalars().all()
        for recipient in recipients:
            send_recipient_task.delay(str(recipient.id))
