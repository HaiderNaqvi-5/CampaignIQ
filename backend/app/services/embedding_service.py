"""
Jina embedding service for CampaignIQ.
...
def get_embedding_provider() -> BaseEmbeddingProvider:
    ...
    if provider_name != "jina" or settings.EMBEDDING_API_KEY in {"", "mock-key", "mock-key-for-development"}:
...
async def embed_chunks_batch(
    ...
    for start in range(0, len(texts), batch_size):
        ...
        batch_embeddings = await provider.embed_texts(batch)
