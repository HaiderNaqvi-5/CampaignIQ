...
def test_campaigniq_models_are_registered_without_legacy_chat_tables():
    tables = set(Base.metadata.tables)

    assert {"users", "bots", "brand_settings", "otp_verifications"} <= tables
    assert {"hubspot_connections", "contacts", "hubspot_lists"} <= tables
    assert {"campaigns", "campaign_steps", "campaign_recipients"} <= tables
...
async def test_cascading_deletion(db_session: AsyncSession):
    """Verify that deleting a user cascades to owned websites."""
    user = User(
    ...
    # Verify the owned website is deleted.
