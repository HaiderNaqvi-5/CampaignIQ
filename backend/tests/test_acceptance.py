...
_otp_codes: dict[str, str] = {}


@pytest.fixture(autouse=True)
def capture_otp(monkeypatch):
    from app.api.v1 import auth
    monkeypatch.setattr(auth, "send_otp_email", lambda email, code: _otp_codes.__setitem__(email, code))
...
async def _register_and_get_token(client: AsyncClient) -> tuple[str, str]:
    ...
    assert res.status_code == 202, f"Register failed: {res.text}"
    verified = await client.post("/api/auth/verify-otp", json={"email": email, "code": _otp_codes[email]})
...
class TestAT001_Authentication:
    ...
    async def test_login_valid_credentials(self, client: AsyncClient):
        ...
        await client.post("/api/auth/verify-otp", json={"email": email, "code": _otp_codes[email]})
...
class TestAT007_HealthCheck:
    ...
    async def test_root_endpoint_returns_200(self, client: AsyncClient):
        ...
        assert res.status_code == 200
        body = res.json()
        assert body["name"] == "CampaignIQ"


# ---------------------------------------------------------------------------
# AT-008: Bot Name Defaults to Domain
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
class TestAT008_BotNameDefault:
